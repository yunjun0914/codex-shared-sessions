    ...
    def test_real_layout_focus_logs_and_restore_on_private_socket(self):
        ...
        with patch.dict(os.environ, env):
            ...
            try:
                ...
                # Check actual split geometry, not only that new panes exist.
                right = lab.pane('main', 'view')
                width = int(lab.tmux('display-message', '-p', '-t', manager, '#{window_width}'))
                right_width = int(lab.tmux('display-message', '-p', '-t', right, '#{pane_width}'))
                self.assertAlmostEqual(right_width, width * 0.55, delta=2)
                top = lab.pane('demo-baseline', 'log-0')
                height = int(lab.tmux('display-message', '-p', '-t', agent, '#{window_height}'))
                top_height = int(lab.tmux('display-message', '-p', '-t', top, '#{pane_height}'))
                self.assertAlmostEqual(top_height, height * 0.45, delta=2)
