...
- Offer Notion conversation backup once after the chosen core setup is verified. Only if requested, read [optional backup onboarding](references/notion-backup.md). The public clone includes the optional `codex-lab-notion` plugin, not installed by default; no private destination or credential is supplied.
